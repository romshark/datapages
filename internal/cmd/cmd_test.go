...
// setupMultiappProject writes a module that builds two applications whose
// events claim one subject, and chdirs into it.
//
// The generated packages are stubbed so that "go mod tidy" resolves the
// imports of the entry point. They carry the generated header,
// which is what lets a later "datapages gen" write over them.
func setupMultiappProject(t *testing.T, alphaSubject, betaSubject string) string {
	t.Helper()
	...
	appSrc := func(pkg, subj, field string) string {
...
func (*App) Head(r *http.Request) datapages.Head { return nil }
...
func (PageIndex) OnPing(event EventPing, sse datapages.SSE) error { return nil }
`, pkg, subj, field)
	}
	writeFileAt(t, filepath.Join(dir, "app", "alpha", "app.go"),
		appSrc("alpha", alphaSubject, "N int `json:\"n\"`"))
	writeFileAt(t, filepath.Join(dir, "app", "beta", "app.go"),
		appSrc("beta", betaSubject, "Secret string `json:\"secret\"`"))

	stub := "// Code generated by github.com/romshark/datapages; DO NOT EDIT.\n\n" +
		"package datapagesgen\n\ntype Server struct{}\n"
	writeFileAt(t, filepath.Join(dir, "app", "alpha", "datapagesgen", "app_gen.go"), stub)
	writeFileAt(t, filepath.Join(dir, "app", "beta", "datapagesgen", "app_gen.go"), stub)

	writeFileAt(t, filepath.Join(dir, "cmd", "server", "main.go"), `package main

import (
	"github.com/romshark/datapages"
	"github.com/romshark/datapages/modules/messaging"

	"testproject/app/alpha"
	alphagen "testproject/app/alpha/datapagesgen"
	"testproject/app/beta"
	betagen "testproject/app/beta/datapagesgen"
)

func newAlpha(a *alpha.App, b messaging.Broker) (datapages.Server, error) {
...
func newBeta(a *beta.App, b messaging.Broker) (datapages.Server, error) {
	return datapages.NewServer[
		beta.App,
		datapages.DisableSessions,
		datapages.DisablePrometheus,
...
// TestLintSubjectAcrossApps tests the subject rule between the applications of
// one module. Each app package is parsed on its own, so a subject two of them
// claim is only visible to a check that has every model of the module in hand.
func TestLintSubjectAcrossApps(t *testing.T) {
	for name, tc := range map[string]struct {
		alphaSubject, betaSubject string
		wantOK                    bool
	}{
		"same subject":      {alphaSubject: "ping", betaSubject: "ping"},
		"different subject": {alphaSubject: "ping", betaSubject: "pong", wantOK: true},
	} {
		t.Run(name, func(t *testing.T) {
			setupMultiappProject(t, tc.alphaSubject, tc.betaSubject)
			...
			require.Equal(t, 1, code, "lint accepted a shared subject")
			out := stderr.String()
			require.Contains(t, out, "app/alpha")
			require.Contains(t, out, "app/beta")
...
func TestLintGen(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		// App type present but errors in other pages: the parser returns a
		// partial model, which gen still writes the generated package from.
		"error partial model": {
