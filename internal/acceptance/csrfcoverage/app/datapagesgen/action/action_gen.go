...
// POSTPageIndexDelete references /delete/
func POSTPageIndexDelete(options ...option) string {
	if len(options) == 0 {
		return "@post('/delete/')"
	}
	var b strings.Builder
	bl, al := actionexpr.BeforeAfterLen(options)
	b.Grow(bl + len("@post('/delete/'") + actionexpr.OptionsLen(options) + len(")") + al)
	actionexpr.WriteBefore(&b, options)
	b.WriteString("@post('/delete/'")
