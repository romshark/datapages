...
import (
	...
	"github.com/romshark/datapages/internal/acceptance/statefulpages/datapagesgen/httperr"
...
// It is checked out of statePoolStateFilters on StreamOpen and returned
...
// allocateStateFilters checks a state value out of statePoolStateFilters, zeroes it, registers it
...
func (s *Server) allocateStateFilters(id string, streamID uint64) *stateSlotStateFilters {
	if !s.stateReserveInstance() {
		return nil
	}
	st := statePoolStateFilters.Get().(*app.StateFilters)
	*st = app.StateFilters{} // total reset; safe reuse across tenants
	slot := &stateSlotStateFilters{state: st, streamID: streamID}
...
// reconnectStateFilters tries to reattach an existing slot (grace-period reconnect)
...
func (s *Server) reconnectStateFilters(id string, streamID uint64) (*stateSlotStateFilters, bool) {
	slot, ok := s.lookupStateFilters(id)
	if !ok {
		return nil, false
	}
	slot.mu.Lock()
...
// closeStreamStateFilters marks id as detached from streamID and starts
...
func (s *Server) closeStreamStateFilters(id string, streamID uint64) {
	slot, ok := s.lookupStateFilters(id)
	if !ok {
		return
	}
		...
		func(
			...
			for msg := range ch {
				...
				case strings.HasPrefix(msg.Subject, EvSubjPrefFiltersUpdated):
					...
					if err := p.OnFiltersUpdated(e, sse, slot.state); err != nil {
...
func (s *Server) handlePageIndexPOSTUpdate(
	...
	err := p.POSTUpdate(r, slot.state, stateID, signals, dispatch)
	if err != nil {
