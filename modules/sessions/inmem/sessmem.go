...
// (e.g. github.com/romshark/datapages/modules/sessions/natskv).
package inmem
...
func New[Data any](tokenGen sessions.TokenGenerator) *SessionManager[Data] {
...
func (m *SessionManager[Data]) CreateSession(
	_ context.Context, rec sessions.Record[Data],
) (string, error) {
	if rec.UserID == "" {
...
) (sessions.Record[Data], error) {
	m.lock.Lock()
	e, exists := m.sessions[token]
	m.lock.Unlock()

	if !exists {
		var zero sessions.Record[Data]
