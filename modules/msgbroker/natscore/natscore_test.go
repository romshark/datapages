...
func subscribe(
	t *testing.T, b *natscore.MessageBroker, m msgbroker.Metrics, subjects ...string,
) msgbroker.MessageBrokerSubscription {
	t.Helper()
...
func publish(
	...
	m msgbroker.Metrics, subject, data string,
) {
	t.Helper()
...
func receive(
	t *testing.T, sub msgbroker.MessageBrokerSubscription,
) msgbroker.Message {
	t.Helper()
	select {
	case msg, ok := <-sub.C():
...
func TestFanOut(t *testing.T) {
	...
	for _, sub := range []msgbroker.MessageBrokerSubscription{first, second} {
...
func TestDefaultChanBuffer(t *testing.T) {
	...
	messages := msgbroker.DefaultBrokerChanBuffer
