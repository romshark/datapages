...
func POSTPageIndexSearch(options ...option) string {
	...
	var b strings.Builder
	bl, al := beforeAfterLen(options)
	b.Grow(bl + len("@post('/search/'") + optionsLen(options) + len(")") + al)
	writeBefore(&b, options)
...
func POSTPagePurchaseConfirm(nameslug string, options ...option) string {
	...
	var b strings.Builder
	bl, al := beforeAfterLen(options)
	b.Grow(bl + len("@post('/shows/") + len(s_nameslug) + len("/purchase/confirm/'") + optionsLen(options) + len(")") + al)
	writeBefore(&b, options)
	b.WriteString("@post('/shows/")
	b.WriteString(s_nameslug)
	b.WriteString("/purchase/confirm/'")
	writeOptions(&b, options)
	b.WriteByte(')')
	writeAfter(&b, options)
	return b.String()
