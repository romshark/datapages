...
A minimal auth demo that implements [`sessions.Manager`](../../modules/sessions/sessions.go) against a local SQLite database via [sqinn-go](https://github.com/cvilsmeier/sqinn-go) (a non-cgo SQLite adapter that drives a small `sqinn` subprocess over stdin/stdout, so `go build` stays pure-Go). The point of the example is to show how a custom session store plugs into the Datapages framework — everything else (user table, bcrypt, templates) is just the smallest surrounding app that exercises it.
