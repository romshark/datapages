...
Read `datapages-state` for per-tab server state.

An event type is dispatched from a handler and delivered over SSE to subscribed
pages. Declare it in the app package or a directly or transitively imported
package; imported types let multiple apps handle the same event.
...
when the publish outlives the handler or needs its own deadline. An action can
use `r.Context()`; an `On` handler has no request parameter and can use
`sse.Context()` for its stream context. Nothing is atomic across dispatchers,
so `errors.Join` their results.
...
Use `datapages.State[T]` for per-tab state read by event handlers.
...
- A `signal:"name"` tag on a `datapages.Subject` field makes the client supply
  ...
  into one literal segment, not a wildcard. The tag value is a period-separated
  signal path; each step matches `[A-Za-z_][A-Za-z0-9_]*` and cannot contain
  `__`. No two fields may share one. A `SubjectUser` field may not carry
  one: it is bound to the authenticated user already.

## Delivery

Delivery is at most once, without replay, to streams subscribed at publish
time. A hidden tab closes its stream by default and misses events. The default
broker subscription buffer holds 16 messages; overflow is dropped without
blocking the publisher or reporting the miss to the page. Render the current
state in full on the next page refresh so missed append or prepend patches do
not leave the view incomplete.

## Stream hooks

`StreamOpen` and `StreamClose` run when a page's SSE stream opens and closes.
Both require `r *http.Request` and at least one of
`streamID datapages.StreamID` or `state datapages.State[T]`, and return
...
`StreamClose` may also take `session`, dispatchers and `stateID string` alongside
`State[T]`: no `sse`, no `signals`.
An error from `StreamOpen` stops setup, closes the stream and goes through
`RecoverError`; one from `StreamClose` is logged.

Use `State[T]` for filters, sort order and other per-tab values. Use stream hooks
for setup and cleanup of resources acquired for a stream. The `streamID` is
internal bookkeeping and must never reach a client.
