...
//
// An abstract page gives its state type to every page that embeds it. The
// pages are still separate and their tabs must not share a value. An app-level
// action must act on the tab that called it, or refuse when the calling tab is
// bound to no page that uses the type.
package app
...
// POSTBump is /bump
//
// An app-level action bound to the shared state type. It works from a tab of
// any page that uses TabContext.
func (a *App) POSTBump(
	_ *http.Request,
	state *TabContext,
	stateID string,
