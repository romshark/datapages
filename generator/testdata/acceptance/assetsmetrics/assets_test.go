...
import (
	...
	"dpacceptance/datapagesgen/assets"
...
// the configuration and reaches both the URL builder and the route. The two
// must agree on it.
...
// TestAssetURLs covers the two generated ways to name an asset. Both are used
// in templates and both must produce the configured prefix.
...
// TestAssetsEscapeTheirDirectory covers a path that climbs out of the embedded
// directory.
...
//
// In dev mode the files come from the directory on disk rather than from the
// binary. An edit to a stylesheet then shows up without a rebuild. This works
// only if the path the generator wrote into the assets package matches where
// the files are, and only if the responses are not cached. A cached stylesheet
// is a change the developer cannot see.
func TestDevModeServesFromDisk(t *testing.T) {
	// The generated code reads dev mode from the environment when the option
	// is applied. The variable is set before the server is built.
...
// TestMetrics covers the instrumentation the Prometheus option adds. The
// counters are read from the registry the server was given, the same registry
// a scrape reads.
...
// TestBrokerMetrics covers the counters the generated code hands the message
// broker. They are what an operator watches to see events flowing, and they
// only move if the generated dispatch passes them along.
func TestBrokerMetrics(t *testing.T) {
	...
	if after := counterTotal(t, "datapages_event_broker_publishes_by_kind_total"); after <= before {
