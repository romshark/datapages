// Package app exercises per-tab state shared through abstract pages: one
// generic abstract page instantiated on two different state types, and one
// plain abstract page whose state type two pages share.
//
...
type App struct {
	// allocations counts how many state values were opened. It shows whether
	// two tabs got one value or two.
...
// StateLabel is the state of PageLabel, reached through the same generic base
// with a different type argument.
type StateLabel struct {
	Text string
}

// Base is a generic abstract page. Every page that embeds Base[T] gets these
// handlers with state *T.
