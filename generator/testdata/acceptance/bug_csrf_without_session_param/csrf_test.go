...
// TestCSRFCoversEveryAction is the recorded failure.
//
// The visitor signs in and then sends the delete action without a CSRF token.
// A cross-site page can make their browser do exactly this. The server must
// refuse it.
func TestCSRFCoversEveryAction(t *testing.T) {
	...
	if status := post("/sign-in/", `{"user":"alice"}`, ""); status != http.StatusOK {
		t.Fatalf("signing in: status = %d", status)
	}

	if status := post("/delete/", `{"confirm":true}`, ""); status != http.StatusForbidden {
	...
	if strings.Contains(string(b), "deleted=1") {
		t.Error("the unprotected action took effect")
	}
}
