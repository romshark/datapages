...
// TestRecoveredActionErrors covers a failed action of a Datastar request. The
// visitor is shown what RecoverError rendered and the request itself succeeds.
...
//
// What the answer says is a separate matter: the status the generated
// fallback tries to send arrives too late, see the
// bug_recover_fallback_status case.
func TestUnrecoveredActionErrorStillAnswers(t *testing.T) {
	srv := newServer(t)

	_, body := post(t, srv, "/unrecoverable/")
	if body == "" {
		t.Error("a request RecoverError could not answer got an empty response")
	}
...
// fails. It is not a Datastar request. There is a browser to navigate, and it
// navigates to the app's own 500 page.
