// Package app exercises action handlers: their methods, their parameters and
// their return values.
//
// Every action records what it received. A test calls the action and then
// reads the record back over a page load. The assertion is that the handler
// ran with the arguments the request carried, not that some string appears in
// generated source.
package app
...
//
// Path variables and query parameters reach actions the same way they reach
// page loads.
...
// POSTPing is /ping
//
// An action on the app rather than on a page. It is reachable from every page
// and belongs to none.
