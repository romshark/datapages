...
func TestMethods(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			srv := newServer(t)
			url := urlOf(t, tt.expr)
			if status, body := srv.call(t, tt.method, url, tt.body); status != http.StatusOK {
				t.Fatalf("%s %s: status = %d, want 200\n%s",
					tt.method, url, status, body)
			}
...
// TestGlobalHead covers the head the app declares once for everything it
// renders, on a page load and on a rendering action alike.
...
// TestRedirect covers the two ways a redirect leaves the server. A Datastar
// request cannot follow a 303: the browser would replace the fragment, not the
// page. It gets a script instead.
func TestRedirect(t *testing.T) {
	...
	// by a plain form post, and that client can only be moved by a real
	// redirect. The status the handler asked for is the status it must send.
...
// TestDatastarOnlyActions covers the actions that cannot serve a plain
// request: reading signals and answering on an SSE connection both require the
// Datastar client.
...
// TestSSEOutput covers an action that writes on the connection of its own
// request. The client sent one request and reads elements and signals back
// from it.
...
// TestSSEOutputCompressed covers the same action as a client that accepts
// compression, which every browser does. The events must survive the
// encoding.
//
// The compressed body ends without its gzip footer, because the response ends
// when the handler returns rather than when the encoder is closed. A reader
// therefore sees every event and then an unexpected EOF. That is tolerated
// here rather than asserted: the events are what the client acts on.
...
func TestActionExpressions(t *testing.T) {
	tests := map[string]struct{ got, want string }{
		...
		// The remaining entry points of the generated action package, with the
		// inputs whose handling is a decision rather than a formatting rule.
